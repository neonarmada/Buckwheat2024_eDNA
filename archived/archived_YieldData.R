...
inflo_data <- read.csv("data/Inflo.csv")
head(inflo_data)
inflo_data1 <- na.omit(inflo_data)

lm00<-lm(Flower~Length+Width,data=inflo_data1)
summary(lm00)
anova(lm00)
coef_00 <- coefficients(lm00)
...
ggplot(data = lm_compare, aes(x = Flower)) +
  geom_point(aes(y = estimate00), colour = "red", alpha = 1/2) +
  geom_point(aes(y = estimate01), colour = "green", alpha = 0) +
  geom_point(aes(y = estimate02), colour = "blue", alpha = 1/2) +
  geom_point(aes(y = estimate03), colour = "orange", alpha = 0) +
  geom_abline(aes(intercept = 0, slope = 1), size = 1) +
  ylim(0, 500) +
  theme_classic()

AIC(lm00, lm01, lm02, lm03)
BIC(lm00, lm01, lm02, lm03)
