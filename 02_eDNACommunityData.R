...
# Community Read Data
set.seed(123) # set seed to get reproducible ordination values
...
sample_data <- read.csv("data/CollectedData_v4.csv") %>%
  select(-Morph)
# ID format A-P01 -> A_P01
sample_data2 <- sample_data %>%
  mutate(ID = str_replace_all(sample_data$ID,"-", "_"))

# Fuzzy Regular expression Join + Sample Data Import
sample_data_joined <- regex_join(SMT_Data, sample_data2, by = "ID", mode = "left") %>%
  select(-ID.x, -ID.y)
rownames(sample_data_joined) <- rownames(community_reads3)
dat <- sample_data(sample_data_joined) # <- Imports as Phyloseq sample data table
...
# Ordination Filtered ========================
# Selection idk what to do for ASV60 ngl
...
ggsave("output/edna_filt_spscores.png",
       plot = spscrs_plot_filt, width = 1250, height = 730, units = "px", dpi = 300)


# Adonis ==================================
# NA Samples
na_values <- sample_data_joined %>%
  filter(is.na(ForestArea))
# Filtering out Blanks and NA values (OTU)
adonis_otu <- comm60 %>%
  filter(rowSums(comm60) != 0)
adonis_otu <- adonis_otu %>%
  filter(!rownames(adonis_otu) %in% rownames(na_values))

# Filtering Out Blanks (Sample data)
adonis_dat <- sample_data_joined %>%
  filter(rownames(sample_data_joined) %in% rownames(adonis_otu))

# Permanova Analysis
permanova.1 <- adonis2(adonis_otu ~ ForestArea * Type * 
                         Variety + Morph - ForestArea:Type:Variety, adonis_dat, perm=9999, by="terms")
permanova.1

write.csv(permanova.1, "output/eDNA_adonis3.csv")

# Yield/Herbiv x Community ================
### Seed Set Linear Model

veg_nmds_result2 <- veg_nmds_result %>%
  rownames_to_column(var = "Samples")
